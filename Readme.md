...
1. If you use ComfyUI-Login extension or https connection, you also need to setup `COMFY_UI_LOGIN_EXTENSION_TOKEN` or `COMFY_TLS` environment variables inside `.env` (or outside, if you will). In this case you may also want to set password on MCWW too, so use `MCWW_AUTH` environment variables
