...
class OptionsUI:
    ...
    def _make_hiddenWorkflows(self):
        ...
        with gr.Row():
            self._components.hiddenWorkflows = gr.Dropdown(label="Hide workflows (wildcards supported, case sensitive, no directories, no .json extension)",
            ...
            with gr.Tabs(elem_classes=["mcww-page-tabs"]):
                ...
                with gr.Tab("Behavior"):
                    with gr.Group():
                        ...
                        self._components.showItemOpenFileFromSystemClipboard = gr.Checkbox(label='Show item "Open file from sys. clipboard" in the clipboard history (if possible)')
                        self._components.restartComfyIfTooLittleGBOfRamIsAvailable = gr.Slider(minimum=0.0, maximum=20.0, step=0.1, label="Automatically restart Comfy server if less then specified number of GiB of RAM is available (avoid OS or terminal crash)")
                        self._components.titleInMediaSession = gr.Checkbox(label="Show title in media session (use when you generate a lot of music)")
                        self._components.protectUrlsInMarkdownOutput = gr.Checkbox(label="Don't open on click links generated by AI in output components (copy to clipboard instead)")
                        ...
                        self._components.holdEscapeToExitUIFullscreen = gr.Checkbox(label="Hold Escape button to exit UI fullscreen (Instead of a regular press, Chromium)")
                with gr.Tab("Layout"):
                    with gr.Group():
